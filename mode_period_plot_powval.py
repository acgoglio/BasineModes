...
for var in modes_vars:
    ...
    for gp in group_centers:
        ...
        pow_fields[f"pow_{gp:.2f}h"][match] = np.fmax(
            pow_fields[f"pow_{gp:.2f}h"][match], pow_data[match]
        )
...
for idx_gp, gp in enumerate(group_centers):
    ...
    plt.contour(nav_lon, nav_lat, masked_pow_pct, levels=[0.005], colors="magenta", linewidths=1.0)
    plt.contourf(nav_lon, nav_lat, tmask, levels=[-1000, 0.05], colors="gray")
    plt.contour(nav_lon, nav_lat, tmask, levels=[0.05], colors="black", linewidths=0.8)
    ...
    plt.title(f"Energy for Mode with Period: {gp:.1f} h ± 0.4 h")
    plt.xlabel("Longitude")
    plt.ylabel("Latitude")
    plt.xlim(-6, 36.3)
    plt.ylim(30, 46)
...
for idx_gp, gp in enumerate(group_centers):
    ...
    im = plt.contourf(nav_lon, nav_lat, masked_pow_pct, levels=np.linspace(0, 100, 41), cmap=cmap_pow_pct)
    plt.colorbar(im, label="Energy (% of abs max)")
    plt.contour(nav_lon, nav_lat, masked_pow_pct, levels=[0.005], colors="magenta", linewidths=1.0)
    plt.contourf(nav_lon, nav_lat, tmask, levels=[-1000, 0.05], colors="gray")
    plt.contour(nav_lon, nav_lat, tmask, levels=[0.05], colors="black", linewidths=0.8)
    ...
    plt.title(f"Absolute Energy for Mode with Period: {gp:.1f} h ± 0.4 h")
    plt.xlabel("Longitude")
    plt.ylabel("Latitude")
    plt.xlim(-6, 36.3)
    plt.ylim(30, 46)
