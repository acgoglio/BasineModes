...
for gp in group_centers:
    ...
    plt.xlim(-6, 36.3)
    plt.ylim(30, 46)
    plt.savefig(os.path.join(output_plot_dir, f"mode_pow_{gp:.1f}h.png"), dpi=300, bbox_inches="tight")
    plt.close()

    # Plot 2: presence mask
    presence_mask = (~np.isnan(mode_field)).astype(int)
    plt.figure(figsize=(10, 4))
    cmap_presence = mpl.colors.ListedColormap(["white", "tab:red"])
    bounds = [0, 0.5, 1.5]
    ...
    plt.xlim(-6, 36.3)
    plt.ylim(30, 46)
    plt.savefig(os.path.join(output_plot_dir, f"mode_flag_pow_{gp:.1f}h.png"), dpi=300, bbox_inches="tight")
    plt.close()
    ...
    cmap_pow_pct = mpl.cm.get_cmap("Greens")
    cmap_pow_pct.set_bad("white")
    im = plt.contourf(nav_lon, nav_lat, masked_pow_pct, levels=np.linspace(0, 100.01, 21), cmap=cmap_pow_pct)
    plt.colorbar(im, label="Energy (% of max)")

    # Contour at 0%
    plt.contour(nav_lon, nav_lat, pow_percent, levels=[0], colors="magenta", linewidths=1.2)

    plt.contourf(nav_lon, nav_lat, tmask, levels=[-1000, 0.05], colors="gray")
    plt.contour(nav_lon, nav_lat, tmask, levels=[0.05], colors="black", linewidths=0.8)
    plt.title(f"Energy for Period: {gp:.1f} h ± 0.5 h")
    plt.xlabel("Longitude")
    plt.ylabel("Latitude")
    plt.xlim(-6, 36.3)
    plt.ylim(30, 46)
    plt.savefig(os.path.join(output_plot_dir, f"mode_powval_{gp:.1f}h.png"), dpi=300, bbox_inches="tight")
    plt.close()
