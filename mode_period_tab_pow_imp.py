...
plt.tight_layout()
plt.savefig(os.path.join(indir, "hist_grouped30min_pow.png"), dpi=300)
plt.show()
...
df_grouped["Grouped_Period"] = df_grouped["Grouped_Period"].astype(float)
df_grouped["%"] = (df_grouped["Count"] / df_grouped["Count"].sum() * 100).round(2)
df_grouped.to_csv(os.path.join(indir, "periods_grouped1h_pow.csv"), index=False)
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(os.path.join(indir, "hist_grouped1h_pow.png"), dpi=300)
plt.show()
